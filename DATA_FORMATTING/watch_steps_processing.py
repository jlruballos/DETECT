#!/usr/bin/env python3
"""
Watch Daily Steps Processing Pipeline

This script processes daily step count data from wearable devices for participants in the DETECT study.
It performs data cleaning, preprocessing, and missing value summaries. Negative step values are removed.

Main steps include:
- Loading and validating the steps dataset
- Dropping rows with negative step counts
- Dropping rows with missing dates or step values
- Converting date strings to datetime.date objects
- Checking and reporting duplicate entries
- Saving summary files and cleaned outputs

Outputs include:
- A cleaned dataset (`watch_steps_cleaned.csv`)
- Step-by-step summaries of subid counts and missingness
"""

__author__ = "Jorge Ruballos"
__email__ = "ruballoj@oregonstate.edu"
__date__ = "2025-07-28"
__version__ = "1.1.0"

import pandas as pd
import os
from datetime import datetime
import sys

program_name = 'watch_steps_processing'

#------ Set up paths ------
base_path = '/mnt/d/DETECT'
output_path = os.path.join(base_path, 'OUTPUT', program_name)
os.makedirs(output_path, exist_ok=True)
steps_path = os.path.join(base_path, 'DETECT_Data', 'Watch_Data', 'Daily_Steps', 'Watch_Daily_Steps_DETECT_2024-12-16.csv')

#------ Load helpers ------
sys.path.append(os.path.join(base_path, 'HELPERS'))
from helpers import preprocess_steps

#------ Logging ------
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
logfile = os.path.join(output_path, f"pipeline_log_{timestamp}.txt")
def log_step(message):
    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    full_msg = f"[{ts}] {message}"
    print(full_msg)
    with open(logfile, "a") as f:
        f.write(full_msg + "\n")

#------ Step 0: Load Data ------
log_step("Loading Watch Steps data.")
df = pd.read_csv(steps_path)

# Count participants and rows
num_subids_0 = df['subid'].nunique()
total_rows_0 = len(df)
log_step(f"Step 0: {num_subids_0} unique subids, {total_rows_0} total rows.")

row_counts_0 = df['subid'].value_counts().sort_index()
row_counts_0.to_csv(os.path.join(output_path, 'row_counts_step_0.csv'))

missing_0 = df.isnull().sum()
missing_0 = missing_0[missing_0 > 0]
missing_0.to_csv(os.path.join(output_path, 'missing_counts_step_0.csv'))

#------ Step 1: Drop negative values ------
log_step("Dropping rows with negative step values.")
df = df[df['steps'] >= 0]

num_subids_1 = df['subid'].nunique()
total_rows_1 = len(df)
log_step(f"Step 1: {num_subids_1} unique subids, {total_rows_1} total rows after removing negatives.")

row_counts_1 = df['subid'].value_counts().sort_index()
row_counts_1.to_csv(os.path.join(output_path, 'row_counts_step_1.csv'))

missing_1 = df.isnull().sum()
missing_1 = missing_1[missing_1 > 0]
missing_1.to_csv(os.path.join(output_path, 'missing_counts_step_1.csv'))

#------ Step 2: Drop rows with missing date or steps ------
log_step("Dropping rows with missing 'date' or 'steps' values.")
df = df.dropna(subset=['date', 'steps'])

num_subids_2 = df['subid'].nunique()
total_rows_2 = len(df)
log_step(f"Step 2: {num_subids_2} unique subids, {total_rows_2} total rows after dropping NAs.")

row_counts_2 = df['subid'].value_counts().sort_index()
row_counts_2.to_csv(os.path.join(output_path, 'row_counts_step_2.csv'))

missing_2 = df.isnull().sum()
missing_2 = missing_2[missing_2 > 0]
missing_2.to_csv(os.path.join(output_path, 'missing_counts_step_2.csv'))

#------ Step 3: Preprocess steps data ------
log_step("Parsing dates and preprocessing steps data.")
df = preprocess_steps(df)

num_subids_3 = df['subid'].nunique()
total_rows_3 = len(df)
log_step(f"Step 3: {num_subids_3} unique subids, {total_rows_3} total rows after date parsing.")

row_counts_3 = df['subid'].value_counts().sort_index()
row_counts_3.to_csv(os.path.join(output_path, 'row_counts_step_3.csv'))

missing_3 = df.isnull().sum()
missing_3 = missing_3[missing_3 > 0]
missing_3.to_csv(os.path.join(output_path, 'missing_counts_step_3.csv'))

#------ Step 4: Check for duplicates ------
log_step("Checking for duplicate (subid, date) entries.")
duplicate_mask = df.duplicated(subset=['subid', 'date'], keep=False)
duplicates = df[duplicate_mask]

if not duplicates.empty:
    duplicate_path = os.path.join(output_path, 'duplicate_entries.csv')
    duplicates.to_csv(duplicate_path, index=False)
    log_step(f"Found {len(duplicates)} duplicate rows. Saved to {duplicate_path}")
else:
    log_step("No duplicate (subid, date) entries found.")

#------ Save Cleaned Data ------
output_cleaned = os.path.join(output_path, 'watch_steps_cleaned.csv')
df.to_csv(output_cleaned, index=False)
log_step(f"Final cleaned data saved to {output_cleaned}")

#------ Final Summary Table ------
summary = pd.DataFrame({
    'unique_subids': [num_subids_0, num_subids_1, num_subids_2, num_subids_3],
    'n_rows':        [total_rows_0, total_rows_1, total_rows_2, total_rows_3]
}, index=['step_0_loaded', 'step_1_no_negatives', 'step_2_drop_na', 'step_3_preprocessed'])

summary.to_csv(os.path.join(output_path, 'summary_pipeline_overview.csv'))
print("Summary saved to summary_pipeline_overview.csv")

#------ Final Missingness Summary ------
missing_all = pd.DataFrame({
    'step_0': missing_0,
    'step_1': missing_1,
    'step_2': missing_2,
    'step_3': missing_3
}).fillna(0).astype(int)

missing_all.to_csv(os.path.join(output_path, 'missing_counts_summary.csv'))
print("Missing value summary saved to missing_counts_summary.csv")
