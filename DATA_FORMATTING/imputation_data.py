...
program_name = 'imputation_pipeline'
...
EMFIT_FEATURES = [
    ...
    'start_sleep_time', 'end_sleep_time'  # Already converted from sin/cos
]
...
for subid in subject_ids:
    ...
    if GENERATE_HISTO:
        ...
        # Generate histograms for each feature showing original vs imputed
        for col in available_imputable_features:
            if col in subject_df.columns:
                try:
                    ...
                    original_data = original_subject_df[col].dropna()
                    if len(original_data) > 0:
                        ax1.hist(original_data, bins=20, alpha=0.7, color='blue', edgecolor='black')
                        ax1.set_title(f'{col} - Original Data (N={len(original_data)})')
                        ax1.set_xlabel('Value')
                        ax1.set_ylabel('Frequency')
                        ax1.grid(True, alpha=0.3)
                    
                    # Imputed data histogram
                    imputed_data = subject_df[col].dropna()
                    if len(imputed_data) > 0:
                        ax2.hist(imputed_data, bins=20, alpha=0.7, color='green', edgecolor='black')
                        ax2.set_title(f'{col} - After Imputation (N={len(imputed_data)})')
                        ax2.set_xlabel('Value')
                        ax2.set_ylabel('Frequency')
                        ax2.grid(True, alpha=0.3)
                    
                    plt.suptitle(f'Subject {subid} - {col} Imputation Comparison')
                    plt.tight_layout()
                    
                    # Save plot
                    plot_path = os.path.join(diagnostic_output_path, f'{subid}_{col}_imputation.png')
                    plt.savefig(plot_path, dpi=150, bbox_inches='tight')
                    plt.close()
                    
                    log_step(f"Generated histogram for {col}")
                    
                except Exception as e:
                    log_step(f"Error generating histogram for {col}: {str(e)}")
                    plt.close()
    ...
    if GENERATE_LINE:
        ...
        # Generate time series plots for each feature
        for col in available_imputable_features:
            if col in subject_df.columns:
                try:
                    ...
                    # Plot original data
                    dates = pd.to_datetime(original_subject_df['date'])
                    original_mask = original_subject_df[col].notna()
                    imputed_mask = original_subject_df[col].isna()
                    
                    # Plot observed values
                    if original_mask.sum() > 0:
                        ax.scatter(dates[original_mask], original_subject_df.loc[original_mask, col], 
                                 color='blue', alpha=0.7, s=20, label='Observed')
                    
                    # Plot imputed values
                    if imputed_mask.sum() > 0:
                        ax.scatter(dates[imputed_mask], subject_df.loc[imputed_mask, col], 
                                 color='red', alpha=0.7, s=20, label='Imputed', marker='x')
                    
                    ax.set_title(f'Subject {subid} - {col} Time Series (Observed vs Imputed)')
                    ax.set_xlabel('Date')
                    ax.set_ylabel('Value')
                    ax.legend()
                    ...
                    # Save plot
                    plot_path = os.path.join(diagnostic_output_path, f'{subid}_{col}_timeseries.png')
                    plt.savefig(plot_path, dpi=150, bbox_inches='tight')
                    plt.close()
                    
                    log_step(f"Generated time series plot for {col}")
                    
                except Exception as e:
                    log_step(f"Error generating time series plot for {col}: {str(e)}")
                    plt.close()
    
    # STEP 7: Create engineered features
    log_step(f"Creating engineered features for subject {subid}")
    log_step(f"subject_df type before feature engineering: {type(subject_df)}")
    log_step(f"subject_df shape before feature engineering: {subject_df.shape}")
    ...
    # Add engineered features to dataframe safely
    if engineered_cols:
        log_step(f"Creating engineered features DataFrame with {len(engineered_cols)} columns")
        engineered_df = pd.DataFrame(engineered_cols, index=subject_df.index)
        log_step(f"subject_df type before concat: {type(subject_df)}, shape: {subject_df.shape}")
        log_step(f"engineered_df type: {type(engineered_df)}, shape: {engineered_df.shape}")
        
        # Ensure subject_df is a DataFrame before concatenation
        if not isinstance(subject_df, pd.DataFrame):
            log_step(f"ERROR: subject_df is {type(subject_df)} before concat, converting to DataFrame")
            subject_df = pd.DataFrame(subject_df)
        
        subject_df = pd.concat([subject_df, engineered_df], axis=1)
        log_step(f"subject_df type after concat: {type(subject_df)}, shape: {subject_df.shape}")
        
        # Verify it's still a DataFrame
        if not isinstance(subject_df, pd.DataFrame):
            log_step(f"ERROR: subject_df became a {type(subject_df)} after feature engineering!")
            # Convert back to DataFrame if it became a Series
            subject_df = subject_df.to_frame().T if hasattr(subject_df, 'to_frame') else pd.DataFrame(subject_df)
    
    # Ensure subject_df is still a DataFrame before tracking
    if not isinstance(subject_df, pd.DataFrame):
        log_step(f"ERROR: subject_df is {type(subject_df)} before tracking step")
        subject_df = pd.DataFrame(subject_df) if hasattr(subject_df, 'index') else pd.DataFrame([subject_df])
    ...
    log_step(f"subject_df type before final preparation: {type(subject_df)}")
    
    # Ensure it's a DataFrame before setting subid
    if not isinstance(subject_df, pd.DataFrame):
        log_step(f"ERROR: Converting {type(subject_df)} to DataFrame in final preparation")
        subject_df = pd.DataFrame(subject_df)
    ...
    if not isinstance(subject_df, pd.DataFrame):
        log_step(f"ERROR: subject_df is {type(subject_df)}, skipping subject {subid}")
        continue
    
    log_step(f"Appending DataFrame with shape: {subject_df.shape}")
...
temporal_cols = []
available_imputable_features = [col for col in IMPUTABLE_FEATURES if col in final_df.columns]
...
# Remove sine/cosine columns from main list since sleep times are already converted
...
# Safely reorder columns - check what columns actually exist
...
# Add days_since columns
for col in final_df.columns:
    if col.startswith('days_since_') and col not in available_cols:
        available_cols.append(col)

# Add label columns
for col in final_df.columns:
    if col.startswith('label_') and col not in available_cols:
        available_cols.append(col)

# Add any remaining columns that might have been missed
for col in final_df.columns:
    if col not in available_cols:
        available_cols.append(col)

log_step(f"Final column ordering: {len(available_cols)} columns")
log_step(f"Columns in final_df: {len(final_df.columns)}")

# Check for missing columns
missing_cols = set(final_df.columns) - set(available_cols)
if missing_cols:
    log_step(f"WARNING: These columns will be dropped: {missing_cols}")

# Reorder final dataframe with only existing columns
