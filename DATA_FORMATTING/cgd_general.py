...
for subid, group in df.groupby('subid'):
    ...
    if len(event_indices) == 0:
        ...
        # Calculate feature summaries for the entire period
        feature_summary = {}
        for feat in feature_cols:
            vals = pd.to_numeric(interval_data[feat], errors='coerce')
            interval_mean = vals.mean()
            # For subjects with no events, use overall mean as baseline
            feature_summary[f"{feat}_interval_mean"] = interval_mean
            feature_summary[f"{feat}_ma7_last"] = vals[-7:].mean() if len(vals) >= 7 else interval_mean
        ...
        # EXISTING LOGIC FOR SUBJECTS WITH EVENTS
        prev_index = 0
        event_num = 0
        ...
        for event_idx in event_indices:
            ...
            for feat in feature_cols:
                vals = pd.to_numeric(interval_data[feat], errors='coerce')
                cumulative_vals = pd.to_numeric(group.loc[prev_index:event_idx - 1, feat], errors='coerce')
...
interval_df.to_csv(f"{output_dir}/intervals_all_participants_{label}.csv", index=False)
print(f"\nExported intervals for ALL participants to: {output_dir}/intervals_all_participants_{label}.csv")
print(f"This includes both participants with events and those without any events.")