#!/usr/bin/env python3
"""
Transitions Data Processing Pipeline

Cleans daily room-to-room transition counts from in-home NYCE sensors for DETECT participants.
The input is expected to contain the columns:
    subid, date, transition_count, multiple_subids, homeid

What this script does:
- Loads the transitions dataset and logs per-subject row counts and missingness
- Removes impossible values (negative `transition_count`)
- Drops rows missing required fields (`date`, `subid`, `transition_count`)
- Parses `date` to a proper calendar date (YYYY-MM-DD)
- Checks and reports duplicate keys (by default: (subid, date); consider adding homeid)
- Writes cleaned data plus step-by-step summaries to OUTPUT/transitions_processing
"""
...
program_name = 'transitions_processing'
base_path = '/mnt/d/DETECT'
output_path = os.path.join(base_path, 'OUTPUT', program_name)
os.makedirs(output_path, exist_ok=True)

activity_path = os.path.join(base_path, 'OUTPUT', 'transitions', 'count_of_transitions.csv')
...
#------ Step 0: Load Data ------
log_step("Step 0: Loading Transitions data.")
...
#------ Step 1: Drop Negative transition_count ------
log_step("Step 1: Dropping rows with negative 'transition_count' values.")
if 'transition_count' in df.columns:
    df = df[df['transition_count'] >= 0]
...
#------ Step 2: Drop rows with missing 'date' or 'subid' or 'transition_count' ------
log_step("Step 2: Dropping rows with missing 'date' or 'subid' or 'transition_count'.")
df = df.dropna(subset=['date', 'subid', 'transition_count'])
...
#------ Save Cleaned Data ------
output_cleaned = os.path.join(output_path, 'transitions_cleaned.csv')
