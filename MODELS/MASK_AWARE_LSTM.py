...
class MaskAwareLSTM(nn.Module):
    ...
    def forward(self, x):
        _, (hn, _) = self.lstm(x)
        return self.fc(hn[-1])  # output raw logits
...
def train_one_fold(model, train_loader, val_loader, epochs, device, fold_num):
    ...
    for epoch in range(epochs):
        ...
        if device.type == 'cuda':
            log_dict[f"Fold{fold_num}/GPU_Memory_MB"] = torch.cuda.memory_allocated() / 1024**2
...
def run_lstm_cv(npz_path, cv_method='groupkfold', n_splits=5,
    ...
    for fold, (train_idx, val_idx) in enumerate(splits):
        ...
        model = MaskAwareLSTM(input_size=X.shape[2], hidden_size=hidden_size).to(device)
